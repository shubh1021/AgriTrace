...
function SetPriceDialog({ batch, user, onPriceSet }: { batch: Batch, user: User, onPriceSet: () => void }) {
  ...
  const onSubmit = (values: SetPriceValues) => {
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        toast({ title: t('success'), description: t('price_set_successfully') });
        onPriceSet();
        setIsOpen(false);
      }
    });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild><Button><Tag className="mr-2" />{t('set_price')}</Button></DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('set_price_for')} {batch.name}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isPending}>
                {isPending ? <Loader2 className="animate-spin" /> : t('confirm_price')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription>{t('my_stock_description')}</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {batches.length === 0 ? <p>{t('no_batches_in_stock')}</p> : batches.map((batch) => (
